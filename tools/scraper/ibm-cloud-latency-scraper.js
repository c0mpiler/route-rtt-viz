 ...
 *   - csv-parser: For parsing any CSV data
...
/**
 * Parse and convert the scraped latency data to our application's format
 */
...
/**
 * Update latency data with the scraped data
 */
async function updateLatencyData(scrapedData) {
  ...
  // Regular processing for the standard paths
  // Load the existing latency data
  ...
  latencyData.connections.forEach(conn => {
    existingConnections.set(`${conn.source}:${conn.target}`, conn);
  });
  
  // Track new and updated connections
  let newCount = 0;
  let updatedCount = 0;
  ...
  for (const conn of newData.connections) {
    ...
    if (existingConn) {
      // Update existing connection if the RTT has changed
      if (existingConn.rtt !== conn.rtt) {
        existingConn.rtt = conn.rtt;
        updatedCount++;
        
        console.log(`Updated: ${conn.source} → ${conn.target} = ${conn.rtt}ms`);
      }
    } else {
      // Add new connection
      latencyData.connections.push(conn);
      newCount++;
      
      console.log(`Added new connection: ${conn.source} → ${conn.target} = ${conn.rtt}ms`);
    }
  }
  
  if (newCount === 0 && updatedCount === 0) {
    console.log('No changes to latency data');
    return false;
  }
  
  console.log(`Added ${newCount} new connections, updated ${updatedCount} existing connections`);
...
/**
 * Update related files (required connections, fallback files)
 */
async function updateRelatedFiles(connections) {
  ...
  if (requiredData) {
    ...
    // Check each new connection against required connections
    for (const conn of connections) {
      ...
      for (const reqConn of matchingConns) {
        if (reqConn.latency !== conn.rtt) {
          ...
          console.log(`Updated required connection: ${reqConn.source} → ${reqConn.target} = ${conn.rtt}ms`);
        }
      }
    }
  ...
  if (fallbackData) {
    ...
    // Check each new connection against fallback connections
    for (const conn of connections) {
      // Look for matching fallback connections
      const matchingConns = fallbackData.connections.filter(
        fbConn => 
          (fbConn.source === conn.source && fbConn.target === conn.target) ||
          (fbConn.source === conn.target && fbConn.target === conn.source)
      );
      
      // Update matching connections
      for (const fbConn of matchingConns) {
        if (fbConn.rtt !== conn.rtt) {
          fbConn.rtt = conn.rtt;
          fallbackUpdated = true;
          
          console.log(`Updated fallback connection: ${fbConn.source} → ${fbConn.target} = ${conn.rtt}ms`);
        }
      }
    }
    
    // Write updated fallback connections if changed
    if (fallbackUpdated) {
  ...
  if (fallbackRequiredData) {
    let fallbackRequiredUpdated = false;
    
    // Check each new connection against fallback required connections
    for (const conn of connections) {
      // Look for matching fallback required connections
      const matchingConns = fallbackRequiredData.connections.filter(
        fbReqConn => 
          (fbReqConn.source === conn.source && fbReqConn.target === conn.target) ||
          (fbReqConn.source === conn.target && fbReqConn.target === conn.source)
      );
      
      // Update matching connections
      for (const fbReqConn of matchingConns) {
        if (fbReqConn.latency !== conn.rtt) {
          fbReqConn.latency = conn.rtt;
          fallbackRequiredUpdated = true;
          
          console.log(`Updated fallback required connection: ${fbReqConn.source} → ${fbReqConn.target} = ${conn.rtt}ms`);
        }
      }
    }
    
    // Write updated fallback required connections if changed
    if (fallbackRequiredUpdated) {
...
async function main() {
  ...
  try {
    ...
    // Update the latency data with the scraped data
    const updated = await updateLatencyData(scrapedData);
    
    if (updated) {
      console.log('Successfully updated latency data');
    } else {
      console.log('No changes to latency data');
    }
