...
const DataExporter: React.FC<DataExporterProps> = ({
  ...
  const generateCSV = useCallback(() => {
    ...
    paths.forEach((path, index) => {
      const pathType = index === 0 ? 'Fastest' : 
                      index === paths.length - 1 ? 'Longest' : 
                      `Alternative ${index}`;
    ...
    return new Promise((resolve, reject) => {
      img.onload = () => {
        ...
        canvas.toBlob((blob) => {
          ...
          a.download = `route-radar-export-${Date.now()}.png`;
        ...
        <div class="section">
          ...
          <div class="summary-grid">
            <div class="summary-card">
              ...
              <p><strong>Total Connections:</strong> ${Object.values(networkData).reduce(
                (sum, destinations) => sum + Object.keys(destinations).length,
                0
              )}</p>
            </div>
          ...
          <div class="section">
            ...
            ${path.hopDetails && path.hopDetails.length > 0 ? `
              <table class="path-table">
                ...
                <tbody>
                  ${path.hopDetails.map((hop, hopIndex) => `
                    <tr>
                      <td>${hopIndex + 1}</td>
                      <td>${hop.source}</td>
                      <td>${hop.target}</td>
  ...
  const handleExport = async (format: ExportFormat) => {
    ...
    try {
      ...
      switch (format) {
        case 'csv':
          content = generateCSV();
          filename = `route-radar-export-${Date.now()}.csv`;
          mimeType = 'text/csv';
          break;
        
        case 'json':
          content = generateJSON();
          filename = `route-radar-export-${Date.now()}.json`;
